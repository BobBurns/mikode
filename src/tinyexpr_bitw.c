...
static te_expr *
new_expr (const int type, const te_expr * parameters[])
{
...
static const te_variable *
find_builtin (const char *name, int len)
{
  int imin = 0;
...
static const te_variable *
find_lookup (const state * s, const char *name, int len)
{
  int iters;
...
factor (state * s)
{
  ...
  while (s->type == TOK_INFIX && (s->function == pow))
      ...
      if (insertion)
	{
	  ...
	  te_expr *insert =
	    NEW_EXPR (TE_FUNCTION2 | TE_FLAG_PURE, insertion->parameters[1],
		      power (s));
	  insert->function = t;
...
te_eval (const te_expr * n)
{
  ...
  switch (TYPE_MASK (n->type))
	...
	case 5:
	  return TE_FUN (int, int, int, int, int) (M (0), M (1), M (2), M (3),
						   M (4));
	case 6:
	  return TE_FUN (int, int, int, int, int, int) (M (0), M (1), M (2),
							M (3), M (4), M (5));
	case 7:
	  return TE_FUN (int, int, int, int, int, int, int) (M (0), M (1),
							     M (2), M (3),
							     M (4), M (5),
							     M (6));
	default:
	...
	case 3:
	  return TE_FUN (void *, int, int, int) (n->parameters[3], M (0),
						 M (1), M (2));
	case 4:
	  return TE_FUN (void *, int, int, int, int) (n->parameters[4], M (0),
						      M (1), M (2), M (3));
	case 5:
	  return TE_FUN (void *, int, int, int, int, int) (n->parameters[5],
							   M (0), M (1),
							   M (2), M (3),
							   M (4));
	case 6:
	  return TE_FUN (void *, int, int, int, int, int,
			 int) (n->parameters[6], M (0), M (1), M (2), M (3),
			       M (4), M (5));
	case 7:
	  return TE_FUN (void *, int, int, int, int, int, int,
			 int) (n->parameters[7], M (0), M (1), M (2), M (3),
			       M (4), M (5), M (6));
	default:
	  return NOTNUM;
	}
...
te_expr *
te_compile (const char *expression, const te_variable * variables,
...
int
te_interp (const char *expression, int *error)
{
