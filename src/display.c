...
int show_reg = 0;
int run_io(uint16_t op, run_state *state, uint8_t **prog, _win *w)
{
	...
	for (i = 0; i < (SCR_MAXX * SCR_MAXY); i++)
	{
		...
		if ((*prog)[i+SCR_OFF] != w->old_win[i]) {
			...
			if ((*prog)[i+SCR_OFF] > 31) {
				mvwaddch(w->screen, i/SCR_MAXX, i%SCR_MAXX, (char)(*prog)[i+SCR_OFF]); 
			}
		}
...
//		wclear(w->text);
		...
		sprintf(reg_str, "op %04x sreg %02x ip:%02x sp:%02x \nr0:%02x r1:%02x r2:%02x "
			"r3:%02x r4:%02x r5:%02x r6:%02x r7:%02x r8:%02x r9:%02x "
			"r10:%02x r11:%02x r12:%02x r13:%02x r14:%02x r15:%02x\n", 
			op, state->sreg, state->ip, state->sp, state->reg[0], state->reg[1], 
			state->reg[2], state->reg[3], state->reg[4], state->reg[5], 
			state->reg[6], state->reg[7], state->reg[8], state->reg[9], 
			state->reg[10], state->reg[11], state->reg[12], state->reg[13], 
