...
int
adc (uint16_t op, run_state * state)
{
  int r, d;
  uint16_t c, result;
  c = state->sreg & C;
...
int
add (uint16_t op, run_state * state)
{
  uint16_t result, r, d;
  r = op & 0x000f;
  d = (op & 0x00f0) >> 4;
...
int
adiw (uint16_t op, run_state * state)
{
...
int
and (uint16_t op, run_state * state)
{
  uint16_t r, d;
  r = op & 0x000f;
  d = (op & 0x00f0) >> 4;
...
int
andi (uint16_t op, run_state * state)
{
...
int
asr (uint16_t op, run_state * state)
{
  uint16_t result, d;
  uint8_t c, s;
  d = (op & 0x00f0) >> 4;
...
int
bclr (uint16_t op, run_state * state)
{
...
int
bld (uint16_t op, run_state * state)
{
...
int
branchs (uint16_t op, run_state * state)
{
  uint8_t b = op & 0x0007;
...
int
branchc (uint16_t op, run_state * state)
{
  uint8_t b = op & 0x0007;
  uint8_t k = (op & 0x03f8) >> 3;
  //printf("k %x\n", k);
...
int
abrk (uint16_t op, run_state * state)
{
  state->ip += 2;
  return 1;
}
...
int
bset (uint16_t op, run_state * state)
{
  uint8_t b = (op & 0x0070) >> 4;
  state->sreg |= (1 << b);
  //printf("bclr result %02x\n", state->sreg);
...
int
bst (uint16_t op, run_state * state)
{
  uint8_t d = (op & 0x00f0) >> 4;
  uint8_t b = (op & 0x0007);
  if (state->reg[d] & (1 << b))
...
int
call (uint16_t op, uint16_t op2, run_state * state, uint8_t ** prog)
{
...
int
clrf (uint16_t op, run_state * state)
{
  /* just debuging */
  /* clear all flags */
  state->sreg = 0;
...
int
cbr (uint16_t op, run_state * state)
{
...
int
com (uint16_t op, run_state * state)
{
  uint8_t d = (op & 0x00f0) >> 4;
...
int
cp (uint16_t op, run_state * state)
{
  int r, d;
  uint16_t result;
  r = op & 0x000f;
...
int
cpc (uint16_t op, run_state * state)
{
  int r, d;
  uint16_t c, result;
  c = state->sreg & C;
...
int
cpi (uint16_t op, run_state * state)
{
  int d;
  uint16_t k, result;
  d = (op & 0x00f0) >> 4;
...
int
cpse (uint16_t op, run_state * state, uint8_t ** prog)
{
  cp (op, state);
  if (!(state->sreg & Z))
    return 0;
...
int
dec (uint16_t op, run_state * state)
{
  int d;
  d = (op & 0x00f0) >> 4;
  if (state->reg[d] == 0x80)
...
int
eor (uint16_t op, run_state * state)
{
  int d, r;
  r = op & 0x000f;
  d = (op & 0x00f0) >> 4;
...
int
icall (uint16_t op, run_state * state, uint8_t ** prog)
{
...
int
ijmp (uint16_t op, run_state * state, uint8_t ** prog)
{
  uint16_t addrh = (state->reg[15] << 8);
  state->ip = addrh | state->reg[14];
  return 0;
...
int
inc (uint16_t op, run_state * state)
{
  int d;
  d = (op & 0x00f0) >> 4;
  /* see spec */
...
int
jmp (uint16_t op, uint16_t op2, run_state * state)
{
  state->ip = op2;
  return 0;
}
...
int
lddx (uint16_t op, run_state * state, uint8_t ** prog)
{
  int rg = (op & 0x00f0) >> 4;
...
int
lddy (uint16_t op, run_state * state, uint8_t ** prog)
{
  int rg = (op & 0x00f0) >> 4;
...
int
ldi (uint16_t op, run_state * state)
{
  int rg = (op & 0x00f0) >> 4;
...
int
lds (uint16_t op, uint16_t op2, run_state * state, uint8_t ** prog)
{
  int rg = (op & 0x00f0) >> 4;
...
int
lsl (uint16_t op, run_state * state)
{
  uint16_t result, d;
  uint8_t c;
  d = (op & 0x00f0) >> 4;
...
int
lsr (uint16_t op, run_state * state)
{
  uint16_t result, d;
  uint8_t c;
  d = (op & 0x00f0) >> 4;
...
int
mov (uint16_t op, run_state * state)
{
  uint16_t r, d;
  r = op & 0x000f;
  d = (op & 0x00f0) >> 4;
...
int
movw (uint16_t op, run_state * state)
{
  uint16_t r, d;
  r = op & 0x000f;
  d = (op & 0x00f0) >> 4;
...
int
mul (uint16_t op, run_state * state)
{
  uint8_t c;
  uint16_t result, r, d;
  r = op & 0x000f;
...
int
muls (uint16_t op, run_state * state)
{
  uint8_t c;
  uint16_t result, r, d;
  r = op & 0x000f;
...
int
neg (uint16_t op, run_state * state)
{
  uint8_t d = (op & 0x00f0) >> 4;
  if (state->reg[d] != 0x80)
...
int
nop (uint16_t op, run_state * state)
{
  state->ip += 2;
  return 0;
}
...
int
or (uint16_t op, run_state * state)
{
  uint16_t r, d;
  r = op & 0x000f;
  d = (op & 0x00f0) >> 4;
...
int
ori (uint16_t op, run_state * state)
{
  uint16_t d = (op & 0x00f0) >> 4;
  state->reg[d] |= (op & 0x000f) | ((op & 0x0f00) >> 4);
  state->sreg &= ~V;
...
int
pop (uint16_t op, run_state * state, uint8_t ** prog)
{
  uint8_t d = (op & 0x00f0) >> 4;
...
int
push (uint16_t op, run_state * state, uint8_t ** prog)
{
  uint8_t d = (op & 0x00f0) >> 4;
...
int
ret (uint16_t op, run_state * state, uint8_t ** prog)
{
...
int
ror (uint16_t op, run_state * state)
{
  uint16_t d;
  uint8_t c;
  d = (op & 0x00f0) >> 4;
...
int
sbc (uint16_t op, run_state * state)
{
  int r, d;
  uint16_t c, result;
  c = state->sreg & C;
...
int
sbci (uint16_t op, run_state * state)
{
  int d;
...
int
sbiw (uint16_t op, run_state * state)
{
  uint8_t rg = 0x08, m = 0, c = 0;
  uint16_t result;
  uint16_t val;
...
int
sbr (uint16_t op, run_state * state)
{
  int d;
  uint16_t val;
  d = (op & 0x00f0) >> 4;
...
int
sbrc (uint16_t op, run_state * state, uint8_t ** prog)
{
  uint8_t d = (op & 0x00f0) >> 4;
  uint8_t b = (op & 0x0007);
  if (!(state->reg[d] & (1 << b)))
...
int
sbrs (uint16_t op, run_state * state, uint8_t ** prog)
{
  uint8_t d = (op & 0x00f0) >> 4;
  uint8_t b = (op & 0x0007);
  if ((state->reg[d] & (1 << b)))
...
int
setf (uint16_t op, run_state * state)
{
  uint8_t b = (op & 0x0070) >> 4;
  state->sreg |= (1 << b);
  state->ip += 2;
...
int
ser (uint16_t op, run_state * state)
{
  uint8_t d = (op & 0x00f0) >> 4;
  state->reg[d] = 0xff;
  state->ip += 2;
...
int
stdx (uint16_t op, run_state * state, uint8_t ** prog)
{
  int rg = (op & 0x00f0) >> 4;
  uint16_t iH = state->reg[13] << 8;
  uint16_t index = iH | state->reg[12];
...
int
stdy (uint16_t op, run_state * state, uint8_t ** prog)
{
  int rg = (op & 0x00f0) >> 4;
  uint16_t iH = state->reg[15] << 8;
  uint16_t index = iH | state->reg[14];
...
int
sts (uint16_t op, uint16_t op2, run_state * state, uint8_t ** prog)
{
  int rg = (op & 0x00f0) >> 4;
...
int
sub (uint16_t op, run_state * state)
{
  int r, d;
  uint16_t result;
  r = op & 0x000f;
...
int
subi (uint16_t op, run_state * state)
{
  int d;
  uint16_t result, val;
  d = (op & 0x00f0) >> 4;
...
int
swap (uint16_t op, run_state * state)
{
  uint8_t d = (op & 0x00f0) >> 4;
...
//              printf("mask: %04x\n", DRD_MSK(op));
      if (DRD_MSK (op) == 0x1c00)
	{
      ...
      else if (DRD_MSK (op) == 0x0c00)
	{
	  err = add (op, state);
	}
      else if (IMP_MSK (op) == 0x9598)
	{
	  err = abrk (op, state);
	}
      else if (DRD_MSK (op) == 0x9600)
	{
	  err = adiw (op, state);
	}
      else if (DRD_MSK (op) == 0x2000)
	{
	  err = and (op, state);
	}
      else if (IMM_MSK (op) == 0x7000)
	{
	  err = andi (op, state);
	  /* special case for cbr */
	}
      else if (DRS_MSK (op) == 0x9405)
	{
	  err = asr (op, state);
	}
      else if (BIT_MSK (op) == 0x9488)
	{
	  err = bclr (op, state);
	}
      else if (LSB_MSK (op) == 0xf800)
	{
	  err = bld (op, state);
	}
      else if (BRCH_MSK (op) == 0xf400)
	{
	  err = branchc (op, state);
	}
      else if (BRCH_MSK (op) == 0xf000)
	{
	  err = branchs (op, state);
	}
      else if (BIT_MSK (op) == 0x9408)
	{
	  err = bset (op, state);
	}
      else if (LSB_MSK (op) == 0xfa00)
	{
	  err = bst (op, state);
	}
      else if (IMM_MSK (op) == 0xc000)
	{
	  err = cbr (op, state);
	}
      else if (IMP_MSK (op) == 0x940e)
	{
      ...
      if (DRS_MSK (op) == 0x9400)
	{
      ...
      else if (DRD_MSK (op) == 0x1400)
	{
	  err = cp (op, state);
	}
      else if (DRD_MSK (op) == 0x0400)
	{
	  err = cpc (op, state);
	}
      else if (IMM_MSK (op) == 0x3000)
	{
	  err = cpi (op, state);
	}
      else if (DRD_MSK (op) == 0x1000)
	{
	  err = cpse (op, state, prog);
	}
      else if (DRS_MSK (op) == 0x940a)
	{
	  err = dec (op, state);
	}
      else if (DRD_MSK (op) == 0x2400)
	{
	  err = eor (op, state);
	}
      else if (IMP_MSK (op) == 0x9509)
	{
	  err = icall (op, state, prog);
	}
      else if (IMP_MSK (op) == 0x9409)
	{
	  err = ijmp (op, state, prog);
	}
      else if (DRS_MSK (op) == 0x9403)
	{
	  err = inc (op, state);
	}
      else if (IMP_MSK (op) == 0x940c)
	{
	  op2 = (*prog)[(state->ip) + 2] | ((*prog)[(state->ip) + 3]) << 8;
	  err = jmp (op, op2, state);
	}
      else if (LDD_MSK (op) == 0x900c)
	{
	  err = lddx (op, state, prog);
	}
      else if (LDD_MSK (op) == 0x9008)
	{
	  err = lddy (op, state, prog);
	}
      else if (IMM_MSK (op) == 0xe000)
	{
	  err = ldi (op, state);
	}
      else if (DRS_MSK (op) == 0x9000)
	{
	  op2 = (*prog)[(state->ip) + 2] | ((*prog)[(state->ip) + 3]) << 8;
      ...
      else if (DRD_MSK (op) == 0x0c00)
	{
      ...
      else if (DRS_MSK (op) == 0x9406)
	{
	  err = lsr (op, state);
	}
      else if (DRD_MSK (op) == 0x2c00)
	{
	  err = mov (op, state);
	}
      else if (DRD_MSK (op) == 0x0100)
	{
	  err = movw (op, state);
	}
      else if (DRD_MSK (op) == 0x9c00)
	{
	  err = mul (op, state);
	}
      else if (DRD_MSK (op) == 0x0200)
	{
	  err = muls (op, state);
	}
      else if (DRS_MSK (op) == 0x9401)
	{
	  err = neg (op, state);
	}
      else if (IMP_MSK (op) == 0x0000)
	{
	  err = nop (op, state);
	}
      else if (DRD_MSK (op) == 0x2800)
	{
	  err = or (op, state);
	}
      else if (IMM_MSK (op) == 0x6000)
	{
      ...
      else if (DRS_MSK (op) == 0x9203)
	{
	  err = pop (op, state, prog);
	}
      else if (DRS_MSK (op) == 0x920f)
	{
	  err = push (op, state, prog);
	}
      else if (IMP_MSK (op) == 0x9508)
	{
      ...
      if (DRS_MSK (op) == 0x9407)
	{
      ...
      else if (DRD_MSK (op) == 0x0800)
	{
	  err = sbc (op, state);
	}
      else if (IMM_MSK (op) == 0x4000)
	{
	  err = sbci (op, state);
	}
      else if (DRD_MSK (op) == 0x9700)
	{
	  err = sbiw (op, state);
	}
      else if (IMM_MSK (op) == 0x6000)
	{
      ...
      else if (LSB_MSK (op) == 0xfc00)
	{
	  err = sbrc (op, state, prog);
	}
      else if (LSB_MSK (op) == 0xfe00)
	{
      ...
      if (DRS_MSK (op) == 0xef0f)
	{
      ...
      else if (LDD_MSK (op) == 0x920c)
	{
	  err = stdx (op, state, prog);
	}
      else if (LDD_MSK (op) == 0x9208)
	{
	  err = stdy (op, state, prog);
	}
      else if (DRS_MSK (op) == 0x9200)
	{
	  op2 = (*prog)[(state->ip) + 2] | ((*prog)[(state->ip) + 3]) << 8;
	  err = sts (op, op2, state, prog);
	}
      else if (DRD_MSK (op) == 0x1800)
	{
	  err = sub (op, state);
	}
