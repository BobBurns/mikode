...
int parse(line *in,uint8_t ***dp, char *b)
{
...
int get_expr_proxy(line *line, int16_t *val, char *b)
{
	int err;
	char *c = b;
	...
	for (j = 0; j < i; j++)
	{
		if (in[j].patch) {
			...
			if (in[j].patch == PATCH_BRANCH) {
				...
				in[j].opcode = in[j].opcode | ((uint16_t)(br_val & 0x007f) << 3);
			} else
			if (in[j].patch == PATCH_DIR32) {
				in[j].opextra = (uint16_t)addr_to; //in[k].address;
			} else
			if (in[j].patch == PATCH_VALL || in[j].patch == PATCH_IMM_BYTE) {
